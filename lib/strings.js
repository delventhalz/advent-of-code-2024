 ...
 * @param {function} [xform] - transform split strings before next split
 * @returns {Array} the split string
 */
export const splitNested = (string, delimiters, xform = identity) => {
  ...
  return string
    .split(delimiters[0])
    .map(item => splitNested(xform(item), nextDelimiters));
};
