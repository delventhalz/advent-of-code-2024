...
/**
 * Repeatedly split a string based on multiple delimiters, producing more and
 * more deeply nested arrays. Unlike the built-in split, this will not attempt
 * a split if a string (or sub-string) does not contain a delimiter.
 *
 * @param {string} string - the string to split
 * @param {Array<string | RegExp>} delimiters - characters to split on
 * @returns {Array | string} the split string
 */
export const splitNested = (string, delimiters) => {
  const validDelimiters = delimiters
    .filter(delim => new RegExp(delim).test(string));

  if (validDelimiters.length === 0) {
    return string;
  }

  const nextDelimiters = validDelimiters.slice(1);

  return string
    .split(validDelimiters[0])
    .map(substr => splitNested(substr, nextDelimiters));
};
