 ...
 */
export const findCoords = (matrix, predicate) => {
...
export const eachOnPath = (matrix, [startX, startY], eachFn, pathFn) => {
  if (hasProp(matrix, startY) && hasProp(matrix[startY], startX)) {
    eachFn(matrix[startY][startX], [startX, startY], matrix);
...
/**
 * Iterates from a starting point to each diagonal spot up and to the right.
 *
 * @param {Array<Array>} matrix - the 2D array
 * @param {number[]} center - the X/Y coordinates of the starting point
 * @param {function} eachFn - the function to call on each diagonal spot
 */
export const eachUpRight = (matrix, coords, eachFn) => {
  eachOnPath(matrix, coords, eachFn, (_, [x, y]) => [x + 1, y - 1]);
};
...
/**
 * Iterates from a starting point to each diagonal spot down and to the right.
 *
 * @param {Array<Array>} matrix - the 2D array
 * @param {number[]} center - the X/Y coordinates of the starting point
 * @param {function} eachFn - the function to call on each diagonal spot
 */
export const eachDownRight = (matrix, coords, eachFn) => {
  eachOnPath(matrix, coords, eachFn, (_, [x, y]) => [x + 1, y + 1]);
};
...
/**
 * Iterates from a starting point to each diagonal spot down and to the left.
 *
 * @param {Array<Array>} matrix - the 2D array
 * @param {number[]} center - the X/Y coordinates of the starting point
 * @param {function} eachFn - the function to call on each diagonal spot
 */
export const eachDownLeft = (matrix, coords, eachFn) => {
  eachOnPath(matrix, coords, eachFn, (_, [x, y]) => [x - 1, y + 1]);
};
...
/**
 * Iterates from a starting point to each diagonal spot up and to the left.
 *
 * @param {Array<Array>} matrix - the 2D array
 * @param {number[]} center - the X/Y coordinates of the starting point
 * @param {function} eachFn - the function to call on each diagonal spot
 */
export const eachUpLeft = (matrix, coords, eachFn) => {
  eachOnPath(matrix, coords, eachFn, (_, [x, y]) => [x - 1, y - 1]);
};

/**
 * Converts a matrix to an array by calling an iterator function on it. The
 * iterator much match the signature:
 *   - matrix
 *   - starting coordinates (in an [x, y] tuple)
 *   - onEach function
 *
 * @param {Array<Array>} matrix - the 2D array
 * @param {function} iteratorFn - function to iterate through matrix cells
 * @param {number[]} [start] - the X/Y coordinates of the starting point
 */
export const arrayFromMatrixIterator = (matrix, iteratorFn, start = [0, 0]) => {
  const items = [];

  iteratorFn(matrix, start, (item) => {
    items.push(item);
  })
 ...
 */
export const stringToMatrix = (map) => map
  .split('\n')
  .map(line => line.split(''));
